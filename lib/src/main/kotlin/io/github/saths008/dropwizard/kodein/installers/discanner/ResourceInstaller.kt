...
class ResourceInstaller : DIScanInstallerInterface {
    override fun search(bindings: BindingsMap): List<Class<*>> = bindings.mapNotNull { (key, _) -> key.isResource() }
}

fun DI.Key<*, *, *>.isResource(): Class<*>? {
    val rawClass = this.type.jvmType as? Class<*>

    if (rawClass != null && rawClass.isAnnotationPresent(Path::class.java)) {
        return rawClass
    }
